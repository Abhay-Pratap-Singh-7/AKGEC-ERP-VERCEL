...
# --- NEW ROUTE FOR PERSONAL DETAILS ---
...
def get_personal_details():
    ...
    headers = {
        ...
        "Referer": f"{BASE_URL}/dashboard/stu"
    }
    ...
    try:
        response = requests.get(f"{BASE_URL}/api/User", headers=headers, params=params, timeout=15)
        if response.status_code == 200:
            return jsonify(response.json()), 200
    except Exception as e:
        return jsonify({"error": str(e)}), 500

    return jsonify({"error": "Failed to fetch personal details"}), 500
